...
import { UpdateAvaliation } from "@/domain/usecases/avaliation";
import { Request, Response } from "express";
import { QueryFailedError } from "typeorm";

export const updateAvaliationController = (usecase: UpdateAvaliation) => async (req: Request, res: Response): Promise<Response<any, Record<string, any>>> => {
    try {
        const data = {
        ...
        const avaliations = await usecase(data);
        return res.json(avaliations).status(200)
    }catch(e: unknown){
